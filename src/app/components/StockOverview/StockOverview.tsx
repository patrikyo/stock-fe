  ...
  const indicator = () => {
    if (data) {
      if (data?.percentChange >= 0) {
        return (
          <span className={Style.positiveIndicator}>
            <FontAwesomeIcon icon={faChevronUp} />
          </span>
        );
      } else {
        return (
          <span className={Style.negativeIndicator}>
    ...
    <>
      ...
      {data && (
        <ul className={Style.stockDetailList}>
          ...
          <li className={Style.stockListItem}>
            {indicator()} {data.percentChange}%
          </li>
          {!data.lastUpdated.includes("00:00:00") && (
            <li className={Style.stockListItem}>
              <span className={Style.stockTimeStamp}>
                hämtades: {data.lastUpdated}
              </span>
            </li>
          )}
          <li className={Style.stockListItem}>
            <button
              className={Style.stockDataFetchBtn}
              onClick={() => {
                setInitialLoad(false);
                getStockInfo(
                  `https://stock-api-dh8r.onrender.com/api/stock/${ticker}`
                );
              }}
            >
              {loadingIndicator()}
          ...
          <li className={Style.stockListItem}>
            <Link
              className={Style.stockLink}
              href={`/stock-detail?ticker=${ticker}`}
            >
